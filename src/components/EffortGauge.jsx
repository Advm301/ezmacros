...
// "Quick Prep Gauge" -- 1 to 3 flames showing how this recipe's effort
// compares to the rest of the app (see utils/effortLevel.js for how the
// tier is actually computed). Everything in this app is already quick, so
// this is relative, not a warning label -- it's meant to help someone
...
export default function EffortGauge({ recipe, size = 12, showLabel = false }) {
  ...
  return (
    ...
    >
      {[1, 2, 3].map((i) => (
        <FlameIcon key={i} size={size} muted={i > level.flames} />
      ))}
      {showLabel && (
